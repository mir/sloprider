...
import { cleanupTempDir, cloneRepo, GitCloneError } from '../repo/clone.ts';
import {
  discoverHooks,
  formatHookAgent,
  installHookBundle,
...
import { discoverMcpServers, type McpCatalogItem } from '../artifacts/mcp.ts';
import { getMcpCapableAgents, mcpAgents } from '../artifacts/mcp.ts';
import { installMcpServerForAgent } from '../artifacts/mcp.ts';
import { addMcpToLock, type McpLockEntry } from '../artifacts/mcp.ts';
import { discoverPlugins } from '../artifacts/plugins.ts';
import {
  addMarketplaceForAgent,
  getPluginCapableAgents,
  installPluginForAgent,
} from '../artifacts/plugins.ts';
import { toCodexEntry, upsertCodexMarketplaceEntry } from '../artifacts/plugins.ts';
import { addPluginToRegistry } from '../artifacts/plugins.ts';
import { hashPluginManifest } from '../artifacts/plugins.ts';
import { parseSource, getOwnerRepo } from '../core/source.ts';
import { addSkillToLock } from '../artifacts/skills.ts';
import { addSkillToLocalLock, computeSkillFolderHash } from '../artifacts/skills.ts';
import { getCommitSha } from '../repo/git-sha.ts';
import {
  discoverSkills,
...
import type { PluginCatalogItem, Skill } from '../core/artifacts.ts';
import type { RepositorySource } from '../core/source.ts';
import type { McpServer } from '../artifacts/mcp.ts';
import type { InstallScope } from '../core/scope.ts';
...
function parseGitSource(input: string): RepositorySource {
...
export function displayMcp(server: McpCatalogItem): string {
  if (server.transport === 'stdio') {
    return [server.command, ...(server.args ?? [])].filter(Boolean).join(' ');
  }
  return server.url ?? '';
}
function stripMcpMetadata(server: McpCatalogItem): McpServer {
  const { configPath: _, ...next } = server;
  return next;
}
function repoPluginSource(parsed: RepositorySource, plugin: PluginCatalogItem) {
  return {
    ...plugin,
    source: {
      source: 'git-subdir' as const,
      url: parsed.url,
      path: plugin.configPath === '.' ? '.' : `./${plugin.configPath}`,
      ref: parsed.ref,
    },
  };
}
function marketplaceSourceDescriptor(parsed: RepositorySource) {
  return {
    source: 'git-subdir' as const,
    url: parsed.url,
...
async function saveDiscoveredMarketplaceSources(
  ...
  for (const name of marketplaceNames) {
    const marketplace: PluginCatalogItem = {
      name,
      category: 'Productivity',
      configPath: parsed.url,
...
export function mcpSourceLabel(server: McpCatalogItem): string {
  return server.configPath;
}
function hookSourceLabel(hook: HookBundleCatalogItem): string {
  return hook.configPath;
}
function locatorLabel(plugin: PluginCatalogItem): string {
...
export function artifactSourceLabel(repoDir: string, artifact: Artifact): string {
  ...
  return locatorLabel(artifact.plugin);
}
...
export function artifactSelectOptions(
  ...
  const mcpsBySource = new Map<string, McpCatalogItem[]>();
  ...
  const hooksBySource = new Map<string, HookBundleCatalogItem[]>();
  ...
  const pluginsBySource = new Map<string, PluginCatalogItem[]>();
  for (const plugin of plugins) {
    const source = locatorLabel(plugin);
...
async function confirmHooks(hooks: HookBundleCatalogItem[]): Promise<void> {
...
export function assertNoDuplicateNames(repoDir: string, artifacts: Artifact[]): void {
  ...
  const mcpsByName = new Map<string, McpCatalogItem[]>();
  ...
  const hooksByAgentName = new Map<string, HookBundleCatalogItem[]>();
  for (const artifact of artifacts) {
  ...
  const pluginsByName = new Map<string, PluginCatalogItem[]>();
  for (const artifact of artifacts) {
  ...
  for (const group of pluginsByName.values()) {
    if (group.length < 2) continue;
    throw duplicateSourceError('plugin', group[0]!.name, group.map(locatorLabel));
  }
}
export async function discoverRepo(source: string): Promise<{
  ...
  let skills: Skill[];
  let mcps: McpCatalogItem[];
  let hooks: HookBundleCatalogItem[];
    ...
    if (successfulAgents.length > 0) {
      ...
      await addPluginToRegistry(
        plugin.name,
        {
          ...
          sourceUrl: parsed.url,
          ref: parsed.ref,
          rootPath: plugin.configPath,
          marketplaceName: plugin.marketplaceName,
          marketplacePath: plugin.marketplacePath,
          installedPath: plugin.configPath,
          manifestHash,
          locator: installPlugin.source,
          sourceSha,
        },
        { global }
      );
...
function printInventory(
  ...
  if (discovered.plugins.length > 0) {
    ...
    for (const plugin of discovered.plugins) {
      ...
      console.log(`  ${plugin.name} - ${locatorLabel(plugin)}${detail}`);
    }
  }
  if (
    discovered.skills.length > 0 ||

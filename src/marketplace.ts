...
import type { AgentType, DiscoveredPlugin, PluginSourceDescriptor } from './types.ts';
...
function parseScope(value: string | undefined): Scope {
  if (!value || value === 'local' || value === 'project') return 'project';
...
function parseMarketplaceArgs(args: string[]): ParsedArgs {
  ...
  if (action !== 'add' && action !== 'list' && action !== 'remove') {
    throw new Error(
      'Usage: sloprider marketplace add <source> --agents codex,claude-code --scope local|global'
    );
  }
...
function sourceDescriptor(source: string): PluginSourceDescriptor {
  const parsed = parseSource(source);

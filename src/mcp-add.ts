...
import { addMcpToLock } from './mcp-lock.ts';
import type { McpInstallResult, McpServer } from './mcp-types.ts';
...
function sameEndpointWithTrailingSlash(source: URL, target: URL): boolean {
  const sourcePath = source.pathname.replace(/\/+$/g, '');
...
function reportInstallResult(result: McpAddInstallResult): void {
  ...
  if (failed.length > 0) {
    ...
    for (const entry of failed) {
      const label = mcpAgents[entry.agent]?.displayName ?? agents[entry.agent].displayName;

...
function locatorFromMarketplace(
  basePath: string,
  ...
  if (typeof source === 'string') {
    ...
    return { configPath: clean || '.', source: { source: 'local', path: source } };
  }
  if (isObject(source)) {
    if (source.source === 'local' && typeof source.path === 'string') {
      const clean = normalizeRepoRelativePath(source.path.replace(/^\.\//, ''));
      return { configPath: clean || '.', source: { source: 'local', path: source.path } };
    }
    if (
      ...
      return {
        configPath: normalizeRepoRelativePath(source.path.replace(/^\.\//, '')),
        source: {
          source: 'git-subdir',
          url: source.url,
          path: source.path,
...
function pluginsFromMarketplace(
  ...
  return plugins.flatMap((entry): PluginCatalogItem[] => {
    if (!isObject(entry)) return [];
    const name = stringValue(entry.name);
    if (!name) return [];
    const source = locatorFromMarketplace(basePath, entry);
    if (!source) return [];
    return [
      {
        name,
...
export async function discoverPlugins(basePath: string): Promise<PluginCatalogItem[]> {
  ...
  const discovered: PluginCatalogItem[] = [];
  ...
  const byName = new Map<string, PluginCatalogItem>();
  for (const plugin of discovered) {
    const key = `${plugin.name.toLowerCase()}:${plugin.configPath}`;
